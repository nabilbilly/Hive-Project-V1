...
def YearsOfExperiences(request):
    if request.method == 'POST':
        selected_years_of_experience_options = request.POST.get('YearsOfExperience_options')  # Get the selected option from POST data
        ...
        user_preference.job_types.set([years_of_experience_options])  # Use set() to update many-to-many
        user_preference.save()
        ...
        return redirect('job-experiences')  # Redirect to the next step
...
def JobExperiences(request):
    if request.method == 'POST':
        ...
        if not selected_job_experience:
            ...
            return render(request, 'Introduction/JobExperience.html', {'JobExperience_options': JobExperience.objects.all()})
        ...
        try:
            job_experience = JobExperience.objects.get(name=selected_job_experience)
